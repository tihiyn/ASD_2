...
class EvenBST {
    ...
    Рефлексия:
    Алгоритм для чётных BST, по сути, ничем не отличается от алгоритма получения сбалансированного деерва из массива,
    за тем лишь исключением, что не нужно сортировать входной массив засчёт in-order обхода.
     */
    public void balance() {
...
class Tree<T>
{
    ...
    Рефлексия:
    Что-то я совсем намудрил со своим решением за O(N^2). Считал количество узлов в поддеревьях правильно, да только
    слишком часто. Достаточно было просто рекурсвино обойти дерево и по окончании обхода получили бы количесвто вершин в
    поддеревьях, а не сразу считать количество вершин на каждой итерации.
     */
